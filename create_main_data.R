...
qaly <- severity_qaly%o%rep(1,121) #outer product to define life long qalys
...
data_Yes_NOAC <- qaly_sampler(
  ...
  size = sim, 
  base_qaly = base_qaly, 
  initial_state = 2, # initial state is suscepctible without any morbidities
  seed = seed,
  end.NOAC.after.bleeding = decision.to.medicate)
...
#################################################################
##               Break data into separate groups               ##
#################################################################

thinline_noac <- simudata_long %>% filter(Group == "NOAC")

thinline_no_noac <- simudata_long %>% filter(Group == "No NOAC")


##################################################################
##                   Control the factor order                   ##
##################################################################

plotdata$Group <- as.factor(plotdata$Group)

plotdata$Group <- fct_rev(plotdata$Group)
